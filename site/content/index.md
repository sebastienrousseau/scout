...
description: "Test any Model Context Protocol server against 93 checks in nine phases. Runs on your machine, so credentials never leave your trust boundary. Every finding cites the request that produced it."
...
readout_score: "84"
readout_grade: "Good"
readout_note: "Nine phases · 93 checks · 50 requests · every finding tied to one of them"
...
faq_one_a: "The Inspector is an excellent place to look around a server by hand — click a tool, read a response, debug an OAuth flow. scout is the non-interactive counterpart: it runs a fixed battery of 93 checks, scores the result, cites its evidence and returns an exit code. Most teams end up using both."
